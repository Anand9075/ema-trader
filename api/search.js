...
async function handler(req, res) {
  try {
    ...
    // For queries >= 3 chars, also try Yahoo Finance with a strict timeout.
    let remote = [];
    if (q.length >= 3) {
      try {
        remote = await Promise.race([
          searchSymbol(q),
          new Promise((_, rej) => setTimeout(() => rej(new Error("timeout")), 3800)),
        ]) || [];
    ...
    return res.status(200).json({ results: searchLocal(req.query?.q || ""), source: "local" });
  }
}
