...
function Pill({ label, active }) {
  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:3, padding:'2px 7px', borderRadius:4, fontSize:10, fontWeight:500,
    ...
    <div className="card" style={{ marginBottom:10 }}>
      ...
      <div style={{ padding:'11px 14px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:38, height:38, borderRadius:8, background:'var(--card2)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:700, color:'var(--accent)', fontFamily:'JetBrains Mono' }}>
            {(pick.name||'?').slice(0,2)}
          </div>
          <div>
...
export default function Scanner() {
  ...
  const picks = (result?.picks || []).filter(p => filter === 'all' ? true : p.confidence?.toLowerCase() === filter);
    ...
    <>
      ...
      {loading && (
        <div className="loading-box" style={{ padding:60 }}>
          ...
          <div style={{ fontSize:11, color:'var(--muted)' }}>Calculating EMA, RSI, ATR, Bollinger, CUSUM, ZigZag</div>
        </div>
      )}
      ...
      {result && !loading && (
        <>
          ...
          {picks.length === 0
            ? <div className="loading-box" style={{ padding:40 }}><div style={{ fontSize:12 }}>No {filter!=='all'?filter+'-confidence ':''} picks — market may be in risk-off regime</div></div>
