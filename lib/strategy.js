...
const sp = (p, len) => Math.min(p, Math.max(2, len - 2));
...
function cusumSignal(prices, k = 0.5, h = 3) {
  try {
    ...
    const slice = prices.slice(-20).filter(Number.isFinite);
    if (slice.length < 10) return 0;
    const mu  = slice.reduce((a, b) => a + b, 0) / slice.length;
    const std = Math.sqrt(slice.map(p => (p - mu) ** 2).reduce((a, b) => a + b, 0) / slice.length) || 1;
    let cp = 0, cn = 0;
    prices.slice(-30).filter(Number.isFinite).forEach(p => {
...
function zzTrend(highs, lows, thresh = 0.03) {
  try {
    if (!highs || !lows || highs.length < 20) return "UNKNOWN";
    const h = highs.slice(-20).filter(Number.isFinite);
    const l = lows.slice(-20).filter(Number.isFinite);
    if (h.length < 10) return "UNKNOWN";
...
function analyseTechnicals(data) {
  try {
    const closes  = (data.close  || []).filter(v => v != null && Number.isFinite(v));
    const highs   = (data.high   || []).filter(v => v != null && Number.isFinite(v));
    const lows    = (data.low    || []).filter(v => v != null && Number.isFinite(v));
    const volumes = (data.volume || []).filter(v => v != null && Number.isFinite(v));
    ...
    try {
      const ml = Math.min(highs.length, lows.length, closes.length);
      if (ml > 15) {
        atrA = ATR.calculate({ period: 14, high: highs.slice(-ml), low: lows.slice(-ml), close: closes.slice(-ml) });
      }
    } catch {}
    try {
      if (closes.length > 25) {
        bbA = BollingerBands.calculate({ period: 20, values: closes, stdDev: 2 });
      }
    } catch {}
    ...
    const volSpike = vol20 > 0 ? ((volumes.at(-1) || 0) / vol20) : 1;

    const bbSqueeze  = bb ? (bb.upper - bb.lower) / (bb.middle || 1) < 0.08 : false;
    ...
    console.warn("[strategy] analyseTechnicals error:", e.message);
    return null;
  }
}

async function scanStock(symbol, opts = {}) {
  try {
    ...
    if (!t) return null;
    if (!t.aboveEMA200)        return null; // HARD: must be above 200 EMA
    ...
    console.warn(`[strategy] scanStock ${symbol} error:`, e.message);
    return null;
  }
}
...
async function runScanner() {
  ...
  const deadline   = start + 20_000;   // 20 s — generous for Vercel Pro, tight for Hobby
  const concurrency = Number(process.env.SCANNER_CONCURRENCY) || 4;
  const failures   = [];
  const results    = [];
  let   index      = 0;

  console.log(`[Scanner] Starting scan of ${SCAN_LIST.length} stocks (concurrency=${concurrency})…`);

  async function worker() {
    while (Date.now() < deadline) {
      const current = index++;
      if (current >= SCAN_LIST.length) return;
      const symbol = SCAN_LIST[current];
      try {
        const remaining = deadline - Date.now();
        if (remaining < 2000) return; // bail if almost out of time
        const pick = await scanStock(symbol, { timeoutMs: Math.min(6000, remaining - 500), retries: 0 });
  ...
  const scanned = Math.min(index, SCAN_LIST.length);

  console.log(`[Scanner] Done in ${elapsed}s — ${picks.length} picks from ${results.length} passed; ${failures.length} failed; ${scanned}/${SCAN_LIST.length} attempted`);
  ...
  return {
    ...
    scanned,
    failures,
    partial:   scanned < SCAN_LIST.length,
    elapsed,
