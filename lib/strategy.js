...
async function scanStock(symbol, opts = {}) {
  const data = await fetchOHLCV(symbol, "1y", "1d", {
    ...
    retries: Number.isFinite(opts.retries) ? opts.retries : 0,
  });
  if (!data) return null;
...
async function runScanner() {
  ...
  const deadline = start + 7_000;
  const concurrency = Number(process.env.SCANNER_CONCURRENCY) || 6;
  const failures = [];
  const results = [];
  let index = 0;
  ...
  async function worker() {
    while (Date.now() < deadline) {
      ...
      try {
        const pick = await scanStock(symbol);
  ...
  console.log(`[Scanner] Done in ${elapsed}s — ${picks.length} picks from ${results.length} passed; ${failures.length} failed`);
  return {
    picks,
    allResults: results,
    totalScanned: SCAN_LIST.length,
    scanned: Math.min(index, SCAN_LIST.length),
    failures,
    partial: index < SCAN_LIST.length,
    elapsed,
